...
public class LzVM
{
	protected final JvmClassLoader loader;
	protected final Map<String, Map<LzCallInsn, Optional<JvmMethod>>> jvmCache = new ConcurrentHashMap<>();
	private LzJCallShutter jcallShutter = LzJCallShutter.ALLOW_EVERYTHING;
	public boolean logInterpretSteps = false;
	
	public LzVM(JvmClassLoader loader)
	{
		this.loader = loader;
	}
	...
	protected JvmMethod findMethod(String owner, LzCallInsn call)
	{
		Map<LzCallInsn, Optional<JvmMethod>> map = jvmCache.computeIfAbsent(owner, k -> new ConcurrentHashMap<>());
		return map.computeIfAbsent(call, c ->
				{
					JvmClass cls = loader.loadClass(owner.replace('/', '.'));
					return Optional.of(cls.getDeclaredMethod(call));
				}
		).orElse(null);
	}
			...
			for(; i < insn.length; i++)
			{
				...
				switch(op)
				{
					...
					case LzOpcodes.JCALL:
					{
						...
						JvmMethod method = findMethod(owner, cinsn);
