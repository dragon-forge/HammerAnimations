...
public class GsonHelper
{
	...
	{
		return (T) (pJson.has(pMemberName) ? convertToObject(pJson.get(pMemberName), pMemberName, pContext, pAdapter)
										   : pFallback);
	}
	...
	public static void writeValue(JsonWriter pWriter, @Nullable JsonElement pJsonElement, @Nullable Comparator<String> pSorter)
			throws IOException
	{
		if(pJsonElement != null && !pJsonElement.isJsonNull())
		{
			...
			} else
			{
				...
				for(Map.Entry<String, JsonElement> entry : sortByKeyIfNeeded(pJsonElement.getAsJsonObject()
						.entrySet(), pSorter))
				{
