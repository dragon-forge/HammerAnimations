...
public abstract class TintInstance
{
			...
			extends TintInstance
	{
		...
		{
			particle.r = Math.lerp(particle.r, (float) this.r.get(), factor);
			particle.g = Math.lerp(particle.g, (float) this.g.get(), factor);
			particle.b = Math.lerp(particle.b, (float) this.b.get(), factor);
